<?php 
    ...
    if(isset($_POST['update']) && !empty($_POST['fname']) && !empty($_POST['lname']) && !empty($_POST['email']) && !empty($_POST['uid']) && isset($_POST['role'])){
        $fname = mysqli_real_escape_string($conn,$_POST['fname']);
        $lname = mysqli_real_escape_string($conn,$_POST['lname']);
        $email = mysqli_real_escape_string($conn,$_POST['email']);
        $role = mysqli_real_escape_string($conn,$_POST['role']);
        $uid = mysqli_real_escape_string($conn,$_POST['uid']);
        $sql = "UPDATE users SET fname = '$fname',lname = '$lname',email = '$email',role = $role WHERE uid = $uid";
...
?>
        ...
        <form autocomplete="off" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="w-50 m-auto bg-light p-3 text-capitalize" method="post">
            ...
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="email" value="<?php echo $rows['email']; ?>" required>
                </div>
            </div>
            <div  class="mb-3 row">
